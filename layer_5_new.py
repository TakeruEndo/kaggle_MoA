...
class Model(nn.Module):
    def __init__(self, num_features, num_targets, hidden_size):
        ...
        self.dense5 = nn.utils.weight_norm(
            nn.Linear(self.hidden_size[3], num_targets))
...
if __name__ == '__main__':
    ...
    data2 = (PCA(n_components=n_comp,
                 random_state=42).fit_transform(data[GENES]))
    train2 = data2[:train_features.shape[0]]
    test2 = data2[-test_features.shape[0]:]

    train2 = pd.DataFrame(
        train2, columns=[f'pca_G-{i}' for i in range(n_comp)])
    ...
    data2 = (PCA(n_components=n_comp,
                 random_state=42).fit_transform(data[CELLS]))
    train2 = data2[:train_features.shape[0]]
    test2 = data2[-test_features.shape[0]:]

    train2 = pd.DataFrame(
        train2, columns=[f'pca_C-{i}' for i in range(n_comp)])
    ...
    folds['kfold'] = scored.fold.astype('int8')

    del scored, drug, targets, vc1, vc2
    import gc
    gc.collect()
    ...
    feature_cols = [c for c in process_data(
        folds).columns if c not in target_cols]
    ...
    LEARNING_RATE = 0.00644275191778923
    WEIGHT_DECAY = 6.8458556531442174e-06
    ...
    def run_training(fold, seed):
        ...
        for epoch in range(EPOCHS):
            ...
            train_loss = train_fn(
                model, optimizer, scheduler, loss_tr, trainloader, DEVICE)
    ...
    sub.to_csv('submission_layer5.csv', index=False)
